...
var IconWrapper = import_react19.memo(function IconWrapper2({
  MyIcon,
  style = {},
  message,
  ...iconProps
}) {
  return !MyIcon ? null : jsx_dev_runtime.jsxDEV("div", {
    ...style,
    children: [
      jsx_dev_runtime.jsxDEV(MyIcon, {
        ...iconProps
      }, undefined, false, undefined, this),
      jsx_dev_runtime.jsxDEV("br", {}, undefined, false, undefined, this),
      message ? jsx_dev_runtime.jsxDEV("span", {
        children: message
      }, "message", false, undefined, this) : null
    ]
  }, undefined, true, undefined, this);
});
// src/Icons/Loading.tsx
var jsx_dev_runtime2 = __toESM(require_jsx_dev_runtime(), 1);
var Loading = (props) => jsx_dev_runtime2.jsxDEV(Icon, {
  ...props,
...
// src/Icons/Offline.tsx
var jsx_dev_runtime3 = __toESM(require_jsx_dev_runtime(), 1);
var Offline = (props) => jsx_dev_runtime3.jsxDEV(Icon, {
  ...props,
...
var Warning = (props) => jsx_dev_runtime4.jsxDEV(Icon, {
  ...props,
  path: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"
}, undefined, false, undefined, this);
// src/Icons/Download.tsx
var jsx_dev_runtime5 = __toESM(require_jsx_dev_runtime(), 1);
var Download = (props) => jsx_dev_runtime5.jsxDEV(Icon, {
  ...props,
...
var unfetch = (url, opts = {}) => {
  return new Promise((resolve, reject) => {
    const options = { ...unfetchOptions, ...opts };
    ...
    function response() {
      ...
      const headers = {};
      let header;
      request.getAllResponseHeaders().replace(/^(.*?):\s*?([\s\S]*?)$/gm, (m, key, value5) => {
        keys.push(key = key.toLowerCase());
        all.push([key, value5]);
        header = headers[key];
        headers[key] = header ? `${header},${value5}` : value5;
      });
      return {
...
var Image2 = import_react20.forwardRef(function Image3({
  theme: theme2,
  loadState,
  width,
  height,
  ...
  const imageProps = import_react20.useMemo(() => {
  ...
  }, [loadState, src, alt, width, height, svgRef, theme2]);
  return loadState === loadStates.Loaded ? jsx_dev_runtime6.jsxDEV("img", {
    ...imageProps
  }, undefined, false, undefined, this) : jsx_dev_runtime6.jsxDEV("svg", {
    ...imageProps
  }, undefined, false, undefined, this);
});
...
// src/idealImageUtils.tsx
var jsx_dev_runtime7 = __toESM(require_jsx_dev_runtime(), 1);
var ssr = () => typeof window === "undefined" || window.navigator.userAgent === "ReactSnap";
var nativeConnection = () => !ssr() && ("connection" in window.navigator);
var guessMaxImageWidth = (dimensions3, w) => {
  if (ssr())
    return 0;
  if (!w) {
    w = window;
  }
  const imgWidth = dimensions3.width;
  const { screen } = w;
...
var selectSrc = ({ srcSet, maxImageWidth }) => {
  switch (srcSet.length) {
    ...
    default: {
      ...
      return filteredSources.length ? filteredSources[0] : sorted.at(-1);
    }
  }
};
...
var defaultGetMessage = (iconKey, imgState) => {
  switch (iconKey) {
    ...
    case iconKeys.Load:
      if (imgState.size) {
        return jsx_dev_runtime7.jsxDEV("span", {
          children: [
            "Click to load (",
    ...
    case iconKeys.Error:
    default:
      return imgState.loadInfo === 404 ? "404. Image not found" : `Error: ${imgState.loadInfo || "unknown"}. Click to reload`;
  }
};
var defaultGetIcon = ({
  imgState,
  ...
  switch (imgState.state) {
    ...
    case loadStates.Loading:
      return networkState.overThreshold ? icons[iconKeys.Loading] : icons[iconKeys.NoIcon];
    case loadStates.Initial:
      if (networkState.onLine) {
        if (imgState.shouldAutoDownload === undefined)
          return icons[iconKeys.NoIcon];
        return imgState.userTriggered || !imgState.shouldAutoDownload ? icons[iconKeys.Load] : icons[iconKeys.NoIcon];
      } else {
        return icons[iconKeys.Offline];
      }
    case loadStates.Error:
      return networkState.onLine ? icons[iconKeys.Error] : icons[iconKeys.Offline];
    default:
      return () => null;
  }
};
...
var defaultStyle = { ...theme.img, ...theme.noscript };
var Noscript = ({
  alt,
  height,
  width,
  ...
  const fallbackParams = import_react21.useMemo(() => {
    if (!srcSet.length || ssr())
      return {};
    return {
      srcSet: srcSet.map((x) => `${getUrl ? getUrl(x) : x.src} ${x.width}w`).join(","),
      src: getUrl ? getUrl(srcSet[0]) : srcSet[0].src
    };
  }, [srcSet, getUrl]);
  return !ssr() ? null : jsx_dev_runtime8.jsxDEV("noscript", {
    children: jsx_dev_runtime8.jsxDEV("img", {
...
var Media = ({
  ...
  const useIconStyle = import_react22.useMemo(() => ({
    width: iconSize + 100,
    height: iconSize,
    color: iconColor
  }), [iconSize, iconColor]);
  const useDivStyles = import_react22.useMemo(() => {
    const background = loadState === loadStates.Loaded ? {} : { backgroundColor: placeholder };
    return {
      ...background,
      ...theme2.placeholder,
      ...style
    };
  }, [loadState, theme2, style, placeholder]);
  const SvgRef = import_react22.useRef(null);
  import_react22.useEffect(() => {
    if (SvgRef.current) {
      onDimensions({
        width: SvgRef.current.clientWidth || SvgRef.current.parentElement?.clientWidth,
        height: SvgRef.current.clientHeight || SvgRef.current.parentElement?.clientHeight
      });
    }
  }, [onDimensions]);
  return jsx_dev_runtime9.jsxDEV("div", {
    ...
    onClick,
    children: [
      jsx_dev_runtime9.jsxDEV(Image2, {
        alt,
        height,
        loadState,
        ref: SvgRef,
      ...
      jsx_dev_runtime9.jsxDEV(Noscript, {
        alt,
        getUrl,
        height,
        srcSet,
      ...
      jsx_dev_runtime9.jsxDEV(IconWrapper, {
        MyIcon: icon,
        message,
        fill: iconColor,
        size: iconSize,
...
var IdealImage = ({
  ...
  const useMotionProps = import_react23.useMemo(() => ({
    initial: { opacity: 0 },
    whileInView: { opacity: 1 },
    viewport: { once: true },
    ...motionProps
  }), [motionProps]);
  const useTheme = import_react23.useMemo(() => {
    return {
      ...theme,
      ...theme2
    };
  }, [theme2]);
  const [withLoader, setLoader] = import_react23.useState();
  const [dimensions3, setDimensions] = import_react23.useState({});
  const [imgState, setImgState] = import_react23.useState({
    loadInfo: 0,
    pickedSrc: {},
  ...
  const [networkState, setNetworkState] = import_react23.useState({
  ...
  const loadStateChange = import_react23.useCallback((loadState, userTriggered, loadInfo = 0) => {
    if (networkState.overThreshold)
      setNetworkState((prevState) => ({
        ...prevState,
        overThreshold: false
      }));
    if (imgState.state !== loadState || imgState.userTriggered !== userTriggered || imgState.loadInfo !== loadInfo)
  ...
  const cancel = import_react23.useCallback((userTriggered) => {
    if (loadStates.Loading !== imgState.state)
      return;
    clear();
    loadStateChange(loadStates.Initial, userTriggered);
  }, [clear, imgState, loadStateChange]);
  const load = import_react23.useCallback((userTriggered) => {
    if (!imgState.url || ssr() || [loadStates.Loading, loadStates.Loaded].includes(imgState.state)) {
      return;
    }
    ...
    newLoader.then(() => {
      ...
      clear();
      loadStateChange(loadStates.Error, false, e.status);
    });
    if (threshold) {
  ...
  const onEnter = (e) => {
    if (imgState.state === iconKeys.Loaded)
      return;
    const pickedSrc = selectSrc({
      srcSet,
      maxImageWidth: srcSet.length > 1 ? guessMaxImageWidth(dimensions3) : 0
    });
    setImgState((prevState) => ({
      ...prevState,
  ...
  import_react23.useEffect(() => {
    ...
    if (nativeConnection()) {
      ...
      window.document.addEventListener("possiblySlowNetwork", possiblySlowNetworkListener, { passive: true });
    }
    updateOnlineStatus();
  ...
  import_react23.useEffect(() => {
    if (imgState.url && ![iconKeys.Loaded, iconKeys.Loading].includes(imgState.state)) {
  ...
  return jsx_dev_runtime10.jsxDEV(motion2.div, {
    onViewportEnter: onEnter,
    onViewportLeave: onLeave,
    ...useMotionProps,
    children: jsx_dev_runtime10.jsxDEV(Media, {
      alt,
      getUrl,
      height,
      icon,
      loadState: imgState.state,
