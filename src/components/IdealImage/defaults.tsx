import { icons, loadStates } from 'react-ideal-image/src/components/constants'
import {
  bytesToSize,
  ssr,
} from 'react-ideal-image/src/components/helpers'
  ...
  switch (icon) {
    ...
    case icons.load:
      ...
      if (imgState.size) {
        return <span>
          Click to load (
            <span style={{whiteSpace: 'nowrap'}}>{bytesToSize(imgState.size)}</span>,
          ),
        </span>
      } else {
